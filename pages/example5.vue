...
const todoListTool = new DynamicTool({
  name: 'listTodos',
...
const toggleTodoTool = new DynamicStructuredTool({
  name: 'toggleTodo',
  description: 'Toggle a todo. The parameter needs to be the createdAt of the task.',
  schema: z.object({
    createdAt: z.number(),
  }),
  func: async (args) => {
    const todo = todos.value.find(({ createdAt }) => createdAt === args.createdAt)
...
const tools = [createTodoTool, todoListTool, toggleTodoTool]

const agentPrompt = ChatPromptTemplate.fromMessages([
  ['system', 'You are a helpful assistant that can help with using a todo application. But you can also assist to make changes in a web application. The application is built using Nuxt 3 and TailwindCSS. Only ask the user for help if you already tried the tools. The user is currently on page example5 (pages/example5.vue). The global styles (including used font) are defined in assets/index.css.'],
  ['human', '{input}'],
...
const generate = async () => {
  ...
  const result = await executor.invoke({ input: prompt.value })
  console.log('result', result)
  response.value = result.output
